...
class IMGModel {
	...
	public function fixit($file){
		if(!file_exists("./fix_tmp")){mkdir("fix_tmp");}
		$id = md5(time().sha1(rnd*255885));
		$tmpfile = "./fix_tmp/".$id.".".pathinfo($file, PATHINFO_EXTENSION);
		copy($file,$tmpfile); 
		$this->load($tmpfile);
		$this->maxarea(1920,1080);
		$this->save($file,IMAGETYPE_JPEG,65);
		unlink($tmpfile);
	}
